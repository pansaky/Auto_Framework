...
class FrameConfig(YamlConfig):
    yaml_path = os.path.join(os.path.expanduser('~/.frame.yaml'))
    _defaults = {
        ...
        'sleep_before_teardown': 18000,
    }

    def __init__(self, yaml_path=None):
        super(FrameConfig, self).__init__(yaml_path or self.yaml_path)
...
class FakeNamespace(YamlConfig):
    ...
    def __getattr__(self, name):
        ...
        log.info("===== config is: {}".format(self._conf))
        log.info("===== attrerro name is: {}".format(name))
...
def _get_config_path():
    system_config_path = '/etc/frame.yaml'
    if not os.path.exists(FrameConfig.yaml_path) and \
            os.path.exists(system_config_path):
        return system_config_path
    return FrameConfig.yaml_path
